<template>
  <el-dialog
    ...
    <el-form-item label="商品的图片" prop="goodsImg">
      <el-input v-model="dataForm.goodsImg" placeholder="商品的图片"></el-input>
    </el-form-item>
    <el-form-item label="商品的详情介绍" prop="goodsDetail">
      <el-input v-model="dataForm.goodsDetail" placeholder="商品的详情介绍"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          goodsDetail: [
            { required: true, message: '商品的详情介绍不能为空', trigger: 'blur' }
          ],
          goodsPrice: [
            { required: true, message: '商品单价不能为空', trigger: 'blur' }
          ],
          goodsStock: [
            { required: true, message: '商品库存, -1表示没有限制不能为空', trigger: 'blur' }
          ]
        }
      }
    },
    methods: {
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/miaosha/goods/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',

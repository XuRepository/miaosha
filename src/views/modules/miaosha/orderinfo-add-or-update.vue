<template>
  <el-dialog
    ...
    <el-form-item label="收货地址ID" prop="deliveryAddrId">
      <el-input v-model="dataForm.deliveryAddrId" placeholder="收货地址ID"></el-input>
    </el-form-item>
    <el-form-item label="" prop="goodsImg">
      <el-input v-model="dataForm.goodsImg" placeholder=""></el-input>
    </el-form-item>
    <el-form-item label="冗余过来的商品名称" prop="goodsName">
      <el-input v-model="dataForm.goodsName" placeholder="冗余过来的商品名称"></el-input>
    </el-form-item>
    <el-form-item label="商品数量" prop="goodsCount">
      <el-input v-model="dataForm.goodsCount" placeholder="商品数量"></el-input>
    </el-form-item>
    <el-form-item label="商品单价" prop="goodsPrice">
      <el-input v-model="dataForm.goodsPrice" placeholder="商品单价"></el-input>
    </el-form-item>
    <el-form-item label="1pc, 2android, 3ios" prop="orderChannel">
      <el-input v-model="dataForm.orderChannel" placeholder="1pc, 2android, 3ios"></el-input>
    </el-form-item>
    <el-form-item label="订单状态, 0新建未支付, 1已支付, 2已发货, 3已收货, 4已退款, 5已完成" prop="status">
      <el-input v-model="dataForm.status" placeholder="订单状态, 0新建未支付, 1已支付, 2已发货, 3已收货, 4已退款, 5已完成"></el-input>
    </el-form-item>
    <el-form-item label="订单的创建时间" prop="createDate">
      <el-input v-model="dataForm.createDate" placeholder="订单的创建时间"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          goodsName: [
            { required: true, message: '冗余过来的商品名称不能为空', trigger: 'blur' }
          ],
          goodsCount: [
          ...
          orderChannel: [
            { required: true, message: '1pc, 2android, 3ios不能为空', trigger: 'blur' }
          ],
          status: [
            { required: true, message: '订单状态, 0新建未支付, 1已支付, 2已发货, 3已收货, 4已退款, 5已完成不能为空', trigger: 'blur' }
          ],
          createDate: [
            { required: true, message: '订单的创建时间不能为空', trigger: 'blur' }
          ],
          payDate: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/miaosha/orderinfo/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/miaosha/orderinfo/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',

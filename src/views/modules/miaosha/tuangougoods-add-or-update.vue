    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/miaosha/tuangougoods/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.goodsId = data.tuangouGoods.goodsId
                this.dataForm.miaoshaPrice = data.tuangouGoods.miaoshaPrice
                this.dataForm.stockCount = data.tuangouGoods.stockCount
                this.dataForm.startDate = data.tuangouGoods.startDate
                this.dataForm.endDate = data.tuangouGoods.endDate
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/miaosha/tuangougoods/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
